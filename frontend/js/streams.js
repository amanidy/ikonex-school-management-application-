...
// Entry point called by app.js when the user clicks "Class Streams".
export async function initStreams() {
  ...
  // "Add Stream" button opens the modal form
...
// If id and name are provided, it's edit mode  form is pre-filled.
function openStreamForm(id = null, name = '') {
  const isEdit = id !== null;
  openModal(isEdit ? 'Edit Stream' : 'Add Class Stream', `
    <div class="form-group">
      <label>Stream Name</label>
      <input type="text" id="stream-name-input"
             placeholder="e.g. Form 1A" value="${name}" />
      <p class="form-hint">Use a clear format like "Form 1A" or "Grade 10B"</p>
    </div>
  ...
  // Save button handler , works for both create and edit
  document.getElementById('save-stream-btn').addEventListener('click', async () => {
    ...
    if (!nameVal) return showToast('Stream name cannot be empty', 'error');
